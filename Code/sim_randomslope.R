...
############################################
#####  four different grouping methods #####
############################################
...
## random slopes
alpha4_h = rnorm(h_tot, mean = 0, sd = 1); alpha4_h = rep(alpha4_h, size)
beta4_h = rnorm(h_tot, mean = 0, sd = 1); beta4_h = rep(beta4_h, size)
kappa4_h = rnorm(h_tot, mean = 0, sd = 1); kappa4_h = rep(kappa4_h, size)
...
am_lin = Xbar*alpha1 + Xrel*alpha2 + V*alpha3 + (U-mean(U))*(alpha4[q] + alpha4_h) + alpha_h 
ps_true = exp(am_lin)/(1+exp(am_lin))
ps_true = ps_true/0.7^-1 + 0.15
trt = rbinom(n_tot, size=1, prob=ps_true)

## generate obs outcome
trteffect_lin = kappa_h + Xbar*kappa1 + Xrel*kappa2 + V*kappa3 + (U-mean(U))^2*(kappa4[m] + kappa4_h)
err_y = rnorm(n_tot, mean=0, sd=1)
y = trt*(trteffect_lin) + 
  beta_h + Xbar*beta1 + Xrel*beta2 + V*beta3 + (U-mean(U))*(beta4[l] + beta4_h) + err_y # Equation 19
...
for(d in 1:dummy.num){
  ...
  if(sum(forktrt)!=0 & sum(forkcon)!=0){
    ...
    if(class(tmp.reg) == "logical") tmp.reg = glm(trt ~ Xbar + Xrel + V, data = tmp.data, family=binomial(link="logit"))
...
#cm = kmeans(cbind(data_obs_cluster$Xbar, data_obs_cluster$V, data_obs_cluster$trt.prop), 10)
...
#cm = kmeans(cbind(data_obs_cluster$Xbar, data_obs_cluster$V), 10)
...
if(!is.na(reg.pooled)){
  ...
  ate.pooled.pooled = sum(data_obs$trt*data_obs$y/data_obs$pooled.ps)/sum(data_obs$trt/data_obs$pooled.ps) - 
  ...
  se.pooled.pooled = sqrt(sigsq1*sum((1/data_obs$pooled.ps[data_obs$trt==1])^2)/(sum(1/data_obs$pooled.ps[data_obs$trt==1]))^2 + 
...
if(sum(is.na(predict(fixed.pooled))) > 0 | is.na(fixed.pooled)){
  ate.pooled.pooled.fixed = NA
  se.pooled.pooled.fixed = NA
}else{
  ...
  ate.pooled.pooled.fixed = sum(data_obs$trt*data_obs$y/data_obs$fixed.ps)/sum(data_obs$trt/data_obs$fixed.ps) - 
    sum((1-data_obs$trt)*data_obs$y/(1-data_obs$fixed.ps))/sum((1-data_obs$trt)/(1-data_obs$fixed.ps))
  
  sigsq1 = var(data_obs$y[data_obs$trt==1])
  sigsq0 = var(data_obs$y[data_obs$trt==0])
  se.pooled.pooled.fixed = sqrt(sigsq1*sum((1/data_obs$fixed.ps[data_obs$trt==1])^2)/(sum(1/data_obs$fixed.ps[data_obs$trt==1]))^2 + 
...
## 2. (full, group)
ate.pooled.group = sum(ate.pool*w.pool, na.rm = TRUE)/sum(w.pool, na.rm = TRUE)
se.pooled.group = sqrt(sum(w.pool^2*sesq.pooled.group, na.rm = TRUE)) / sum(w.pool, na.rm = TRUE)

## 3. (full, cluster)
overlap.cl.count = sum(!is.na(ate.h)) # number of overlapping clusters in simulation.
ate.pooled.cluster = sum(ate.h*w.h, na.rm = TRUE)/sum(w.h, na.rm = TRUE)
se.pooled.cluster = sqrt(sum(w.h^2*sesq.pooled.cluster, na.rm = TRUE)) / sum(w.h, na.rm = TRUE)

## 4. (group, full)
ate.group.pooled = sum(data_obs$trt*data_obs$y/data_obs$group.ps, na.rm = TRUE)/sum(data_obs$trt/data_obs$group.ps, na.rm = TRUE) - 
  sum((1-data_obs$trt)*data_obs$y/(1-data_obs$group.ps), na.rm = TRUE)/sum((1-data_obs$trt)/(1-data_obs$group.ps), na.rm = TRUE)
se.group.pooled = sqrt(sigsq1*sum((1/data_obs$group.ps[data_obs$trt==1])^2, na.rm = TRUE)/(sum(1/data_obs$group.ps[data_obs$trt==1], na.rm = TRUE))^2 + 
